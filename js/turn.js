...
/**
 * Registra gli esiti della partita completata
 */
function recordGameOutcome() {
  const winner = determineSingleGameWinner(calculateSingleGameScores());
  const scores = calculateSingleGameScores();
  
  recordGameStats(winner);
  
  // Notifica finale con suono
  playSound("hand-end");
  notifyGameEnd(winner, scores.me, Math.max(scores.ai1, scores.ai2));
  
  // Log statistiche in console
  console.log(displaySessionStats());
}
